...
// const redlock = new Redlock([redis], { retryCount: 0 }); // retryCount:0 = fail fast for demo
...
app.post('/:id/buy', async (req, res) => {
    ...
    try {
        // lock = await redlock.acquire(['lock:stock'], 5000);
        ...
        // console.log(`[${INSTANCE}] Lock denied ❌`);
        // res.status(423).json({ error: 'Server busy, try again.', instance: INSTANCE });
    } finally {
        // await lock?.release();
